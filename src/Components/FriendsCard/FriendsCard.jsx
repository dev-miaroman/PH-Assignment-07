...
const FriendsCard = ({ friendInfo }) => {
  ...
  return (
    <Link href={`/friendsdetails/${friendInfo.id  }`}>
      <div className="bg-white p-10 rounded-lg shadow border-2 border-zinc-100 hover:border-[#244d3f] flex flex-col items-center justify-center gap-5 hover:shadow-[#244d3f]-300">
        ...
        <div className="flex flex-col items-center justify-center space-y-2.5">
          <div className="flex items-center justify-center gap-2.5">
            {tags.map((tag, i) => (
              <span
                key={i}
                className="px-5 py-1.5 bg-[#244d3f] text-white font-medium uppercase rounded-full"
              >
                {tag}
              </span>
            ))}
